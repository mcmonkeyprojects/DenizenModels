...
dmodels_spawn_model:
    ...
    - if <[fake_to].exists>:
        - fakespawn dmodel_part_display[scale=<location[0,0,0]>] <[center]> players:<[fake_to]> save:root d:infinite
        - define root <entry[root].faked_entity>
    - else:
        - spawn dmodel_part_display[scale=<location[0,0,0]>] <[center]> save:root
